...
public interface GameRepository extends JpaRepository<Game,Long> {
    ...
    @Query(nativeQuery=true,
            value="SELECT (SUM(correct_answers)*100/SUM(number_of_questions)*100)/100 FROM games WHERE user_email=:userEmail AND category_id=:categoryId AND date > CURRENT_DATE - :daysToBack")
    Float getPercentGoodAnswers(@Param("userEmail")String userEmail, @Param("categoryId")String categoryId,
                                @Param("daysToBack") int daysToBack);
